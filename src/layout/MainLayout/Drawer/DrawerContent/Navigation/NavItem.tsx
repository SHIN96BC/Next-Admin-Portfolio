...
// types
import { MenuOrientation, ThemeMode } from '@Src/types/config';
import { LinkTarget, NavActionType, NavItemType } from '@Src/types/menu';
...
const NavItem = ({ item, level, isParents = false }: Props) => {
  ...
  // active menus item on page load
  useEffect(() => {
    if (pathname === item.url) updateOpenedItem(item.id!);
      ...
      {menuOrientation === MenuOrientation.VERTICAL || downLG ? (
        <Box sx={{ position: 'relative' }}>
          <ListItemButton
            ...
            {...(downLG && {
              onClick: () => updateMainDrawerOpened(false)
            })}
          >
            {itemIcon && (
              <ListItemIcon
        ...
        <ListItemButton
          ...
          {...(isParents && {
            onClick: () => {
              updateOpenedHorizontalItem(item.id!);
            }
          })}
          sx={{
            zIndex: 1201,

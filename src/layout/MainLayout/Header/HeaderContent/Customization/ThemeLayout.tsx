...
// types
import { MenuOrientation, ThemeDirection } from '@Src/types/config';
...
const miniMenu = '/assets/images/customization/mini-menus.svg';

// ==============================|| CUSTOMIZATION - LAYOUT ||============================== //

const ThemeLayout = () => {
  const theme = useTheme();
  const {isMainDrawerOpened: drawerOpen, updateMainDrawerOpened} = useMenu();
  ...
  const handleRadioChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (newValue === 'default') {
      ...
      if (!drawerOpen) {
        updateMainDrawerOpened(true);
      }
    }
    if (newValue === 'mini') {
      onChangeMiniDrawer(true);
      if (drawerOpen) {
        updateMainDrawerOpened(false);
      }
    }
